...
//        Date: 8/24/2023
//     Problem: Make a grocery list that the user can edit (add/delete), which they can display at any time.
// Description: Program uses singularly linked list, and the user can add, delete, and display strings in
//              the list. Program will determine if string already exists and will not allow duplicate strings.
//************************************************************************************************************
...
// function checks if any nodes have a duplicate value as user input
...
bool duplicateNode(node *head, string userInput)
{
  ...
  // make current pointer and initialize it with head value
  node* current = head;
...
// function will call duplicateNode() to not allow duplicate values to be added
void addNode(node *&head)
{
  ...
  // function call checks to see if there is duplicate value in program
  if (duplicateNode(head, userInput))
    cout << "Duplicate item found. Cannot add to list.\n" << endl;

  // statement if no duplicate value is found in program
  else
  {
    ...
    // sets newNode as first value of list if program finds that there are no oher nodes 
    if (head == NULL)
      head = newNode;
...
// function will delete nunmber in list that user wants deleted.
// but if user input cannot be found in list, no numbers will be deleted.
void deleteNode(node *&head)
{
  ...
  // if head is null, program cannot delete any numbers
  if (head == NULL)
    cout << "List is empty. Cannot delete any items." << endl;

  // list contains nodes, so program will search for number
  else
  { 
    // ask for user input
    ...
    // statement shows that previous pointer is the only number in list
